...
function isValidSentence(sentence) {
    ...
    for (let word of forbiddenStart) {
        if (lower.startsWith(word)) return false;
    }
    if (/hey\W*beautiful/gi.test(trimmed)) return false;

    const foodBlacklist = [
        "pizza", "burger", "sandwich", "spaghetti", "curry", "sushi", "taco", "burrito",
        "noodle", "rice", "chicken", "beef", "pork", "cake", "cookie", "ice cream", "chocolate", "apple pie", "steak"
    ];
    for (let food of foodBlacklist) {
        if (lower.includes(food)) return false;
    }
    ...
    for (const meaning of meanings) {
        ...
        if (definitions) {
            for (const def of definitions) {
                if (def.example && isValidSentence(def.example)) {
                    return def.example;
                }
            }
        }
...
export async function fetchOxfordExample(word) {
    ...
    try {
        ...
        console.warn(`获取单词 ${word} 的例句失败:`, err);
        return null;
    }
}
